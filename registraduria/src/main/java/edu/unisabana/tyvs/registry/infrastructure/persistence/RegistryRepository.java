...
public class RegistryRepository implements RegistryRepositoryPort {
    ...
    /**
     * Si no es null, las conexiones salen de este pool y se DEVUELVEN al cerrarlas.
     * Si es null (constructores por URL, usados por las pruebas unitarias) se
     * mantiene el comportamiento original: una conexion nueva por operacion.
     */
    private final DataSource dataSource;

    /** Constructor con pool: es el que usa la aplicacion (ver RegistryConfig). */
    ...
    private Connection getConnection() throws SQLException {
        // CORRECCION PERF: antes SIEMPRE se abria una conexion nueva por operacion
        // (DriverManager). Con un pool, close() en el try-with-resources devuelve
        // la conexion al pool en vez de destruirla.
        if (dataSource != null) {
